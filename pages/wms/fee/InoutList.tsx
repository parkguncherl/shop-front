...
const InoutList = () => {
  ...
  const [filters, onChangeFilters] = useFilters({
    ...
    orderFee: 0, //제작요율 %
    maintFee: 0, //보관수수료 건당
    hangerFee: 0, //보관수수료 행거 건당
  });

  useEffect(() => {
    if (filters.feeType == 'A') {
      onChangeFilters('stockFee', 0); // 입고수수료 건당
      onChangeFilters('jobFee', 0); // 출고수수료 건당
      onChangeFilters('serviceFee', 7); // 일반요율 %
      onChangeFilters('orderFee', 7); // 제작요율 %
      onChangeFilters('maintFee', 0); // 보관수수료 건당
      onChangeFilters('hangerFee', 0); // 보관수수료 행거 건당
    } else if (filters.feeType == 'B') {
      onChangeFilters('stockFee', 100); // 입고수수료 건당
      onChangeFilters('jobFee', 200); // 출고수수료 건당
      onChangeFilters('serviceFee', 1); // 일반요율 %
      onChangeFilters('orderFee', 1); // 제작요율 %
      onChangeFilters('maintFee', 10); // 보관수수료 건당
      onChangeFilters('hangerFee', 15); // 보관수수료 행거 건당
    } else if (filters.feeType == 'C') {
      onChangeFilters('stockFee', 100); // 입고수수료 건당
      onChangeFilters('jobFee', 200); // 출고수수료 건당
      onChangeFilters('serviceFee', 0.1); // 일반요율 %
      onChangeFilters('orderFee', 0.1); // 제작요율 %
      onChangeFilters('maintFee', 1); // 보관수수료 건당
      onChangeFilters('hangerFee', 1.5); // 보관수수료 행거 건당
    }
  }, [filters.feeType]);
          ...
          const {
            ...
            (
              ...
              return {
                ...
                totalFeeAmt: acc.totalFeeAmt + (data.totalFeeAmt ? data.totalFeeAmt : 0),
