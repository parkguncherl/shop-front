// pages/wms/inven/components/invensku.tsx
...
const InvenSku: React.FC<InvenSkuProps> = ({ filters, paging, setPaging, searchTrigger }) => {
  ...
  const {
    ...
    ['/wms/inven/sku/paging', paging.curPage, paging.pageRowCount, searchTrigger], // filters 추가
    () =>
  ...
  const handleTopGridCellClick = async (event: any) => {
    ...
    try {
      ...
      const response = await authApi.get(`/wms/inven/sku/${event.data.id}`);
