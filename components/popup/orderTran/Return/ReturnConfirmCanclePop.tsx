...
/** 확정 취소 */
const ReturnConfirmCanclePop = ({ selectedData }: Props) => {
  ...
  const { mutate: updateReturnMutate, isLoading } = useMutation(updateReturn, {
    onSuccess: async (e) => {
      if (e.data.resultCode === 200) {
        toastSuccess('반납확정이 취소되었습니다.');
        await queryClient.invalidateQueries(['/store/confirm/detail']);
        await queryClient.invalidateQueries(['/store/confirm/list']);
  ...
  const handleSubmit = () => {
    ...
    const allRowData = gridRef.current?.api.getRenderedNodes().map((node: any) => {
      ...
      rowData.updateType = 3; // 확정취소

      return rowData;
    });

    // 각 행에 대해 반납확정 수와 반납취소 수 비교
    const errorOccurred = allRowData?.some((rowData) => {
      const skuCnt = rowData.skuCnt || 0; // 반납확정 수
      const updateSkuCnt = rowData.updateSkuCnt || 0; // 반납취소 수

      // 반납취소 수가 반납확정 수보다 크면 오류 처리
      if (updateSkuCnt > skuCnt) {
        toastError('반납취소 수가 반납확정 수보다 많을 수 없습니다.');
        return true; // 오류 발생 시 반복을 종료합니다.
      }
      return false;
    });

    // 오류가 발생하면 이후 코드를 실행하지 않음
  ...
  return (
    <PopupLayout
      width={500}
      isEscClose={true}
      open={modalType.type === 'CONFIRMCANCLE'}
      title={'확정 취소'}
      onClose={() => {
        closeModal('CONFIRMCANCLE');
      }}
      footer={
        <PopupFooter>
          <div className="btnArea">
            ...
            <button className="btn" title="닫기" onClick={() => closeModal('CONFIRMCANCLE')}>
              취소
            </button>
          </div>
        </PopupFooter>
...
export default ReturnConfirmCanclePop;
