...
/** 결제거래 팝업 */
export const MichulOrderEditPop = () => {
  ...
  /** 금일내역 스토어 - State */
  const [openModal, modalType, closeModal] = useMichulStore((s) => [s.openModal, s.modalType, s.closeModal]);
  ...
  return (
    <PopupLayout
      width={630}
      isEscClose={true}
      open={modalType.type === 'ORDEREDIT'}
      title={'주문 수정하기'}
      onClose={() => {
        closeModal('ORDEREDIT');
      }}
      footer={
        <PopupFooter>
          <div className="btnArea">
            <button className="btn" title="닫기" onClick={() => closeModal('ORDEREDIT')}>
              닫기
            </button>
          </div>
        </PopupFooter>
