...
const TotSailListPop: React.FC<TotSailListPopProps> = ({ skuId, skuNm, startDate, endDate }) => {
  ...
  } = useQuery(['/product/productTranData/skuTotal'], (): any =>
    authApi.get('/product/productTranData/skuTotal', {
      params: {
        startDate: startDate,
        endDate: endDate,
        skuId: skuId,
  ...
  return (
    <PopupLayout
      width={600}
      height={600}
      open={modalType.type === 'TOTSAIL' && modalType.active}
      title={skuNm}
      onClose={() => {
        closeModal('TOTSAIL');
      }}
      footer={
        <PopupFooter>
          <div className="btnArea">
            <button className="btn" title="닫기" onClick={() => closeModal('TOTSAIL')}>
              닫기
            </button>
          </div>
        </PopupFooter>
