import React, { useEffect } from 'react';
import { PopupFooter } from '../PopupFooter';
import { PopupLayout } from '../PopupLayout';
import { PopupContent } from '../PopupContent';
import TunedGrid from '../../grid/TunedGrid';
import { MisongResponseGroupBySku, ProductResponseSailInfoList } from '../../../generated';
import { defaultColDef, GridSetting } from '../../../libs/ag-grid';
import { useQuery } from '@tanstack/react-query';
import { authApi } from '../../../libs';
import { toastError } from '../../ToastMessage';
import { Utils } from '../../../libs/utils';
import { ColDef, RowClassParams } from 'ag-grid-community';
import { useProductTranDataStore } from '../../../stores/useProductTranDataStore';

interface DcSailListPopProps {
  startDate: string;
  endDate: string;
  skuId: number;
  skuNm: string;
}

const DcSailListPop: React.FC<DcSailListPopProps> = ({ skuId, skuNm, startDate, endDate }) => {
  const [modalType, openModal, closeModal] = useProductTranDataStore((s) => [s.modalType, s.openModal, s.closeModal]);
  /** 컬럼 정의 */
  const SkuListColsForPop: ColDef<ProductResponseSailInfoList>[] = [
    {
      field: 'no',
      headerName: 'No',
      minWidth: 50,
      maxWidth: 50,
      suppressHeaderMenuButton: true,
      cellStyle: GridSetting.CellStyle.CENTER,
    },
    {
      field: 'sellerNm',
      headerName: '소매처',
      width: 200,
      suppressHeaderMenuButton: true,
      cellStyle: GridSetting.CellStyle.LEFT,
    },
    {
      field: 'dcAmt',
      headerName: '단가DC',
      minWidth: 70,
      maxWidth: 70,
      suppressHeaderMenuButton: true,
      cellStyle: {
        textAlign: 'right',
        paddingRight: '8px', // 원하는 만큼 조절
      },
      valueFormatter: (params) => {
        return Utils.setComma(params.value);
      },
    },
    {
      field: 'dcYn',
      headerName: '품목전체',
      width: 80,
      suppressHeaderMenuButton: true,
      cellStyle: GridSetting.CellStyle.CENTER,
    },
  ];

  /** 조건에 해당하는 상품 데이터(row) 조회 */
  const {
    data: skuDataList,
    isSuccess,
    refetch: fetchSkuList,
  } = useQuery(['/product/productTranData/skuDc'], (): any =>
    authApi.get('/product/productTranData/skuDc', {
      params: {
        startDate: startDate,
        endDate: endDate,
        skuId: skuId,
      },
    }),
  );

  useEffect(() => {
    if (isSuccess) {
      console.log('skuDataList ===>', skuDataList);
      const { resultCode, body, resultMessage } = skuDataList.data;
      if (resultCode === 200) {
        console.log('resultCode ===>', resultCode);

        // 합계 데이터 시작
        if (body && body.length > 0) {
          console.log('body ===>', body);
        }
      } else {
        toastError(resultMessage);
      }
    }
  }, [isSuccess, skuDataList]);

  return (
    <PopupLayout
      width={600}
      height={600}
      open={modalType.type === 'DCSAIL' && modalType.active}
      title={skuNm}
      onClose={() => {
        closeModal('DCSAIL');
      }}
      footer={
        <PopupFooter>
          <div className="btnArea">
            <button className="btn" title="닫기" onClick={() => closeModal('DCSAIL')}>
              닫기
            </button>
          </div>
        </PopupFooter>
      }
    >
      <PopupContent>
        <TunedGrid<MisongResponseGroupBySku>
          colIndexForSuppressKeyEvent={2}
          headerHeight={35}
          rowData={(skuDataList?.data.body as ProductResponseSailInfoList[]) || []}
          columnDefs={SkuListColsForPop}
          defaultColDef={defaultColDef}
          gridOptions={{
            rowHeight: 24,
          }}
          className={'pop'}
        />
      </PopupContent>
    </PopupLayout>
  );
};

export default DcSailListPop;
